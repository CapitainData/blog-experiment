...
# Data, Science, Learning Techniques and Algorithms
</div>
...
ggplot(aes(x = x, y = value), data = gdat) +
  geom_ribbon(aes(ymin = se_lower, ymax = se_upper, group = variable),
              fill = 'gray98') +
  geom_line(aes(group = variable), color = '#FF55DD80') +
...
ggplot() +
  geom_ribbon(aes(x = X_test, ymin = lb, ymax = ub), fill = 'brown90') +
  geom_line(aes(x = X_test, y = mu), size = 1, color = '#ff55ff80') +
  geom_point(
    aes(x = X_train, y = y_train),
    size = 4,
    color = '#4485a1',
